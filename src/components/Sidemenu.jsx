...
const Sidemenu = () => {
  ...
  useEffect(() => {
    ...
    if (!isHovering) {
      interval = setInterval(() => {
        setActiveIndex((prevIndex) => (prevIndex + 1) % 6); // Now for 6 menu items
      }, 5000); // Change menu item every 5 seconds (3s display + 2s delay)
    }
  ...
  const handleMouseLeave = () => {
    setIsHovering(false);
    setTimeout(() => {
      setActiveIndex(-1); // Reset after 2 seconds delay
    }, 2000);
  };
      ...
      <div className="fixed z-20">
        {menuItems.map((item, index) => (
          <Link
            ...
            <div
              className={`flex gap-2 fixed top-[${
                300 + index * 40
              }px] right-0 hover:cursor-pointer font-extrabold ${
                activeIndex === index ? "expand" : ""
              }`}
              onMouseEnter={() => handleMouseEnter(index)}
              onMouseLeave={handleMouseLeave}>
              <span
                className="inline-flex items-center p-2 bg-[#00664a] text-white group transition-all duration-700 ease-in-out focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:outline-none"
                role="alert"
                tabIndex="0">
                {item.svg}
                <span className="whitespace-nowrap inline-block group-hover:max-w-screen-2xl group-focus:max-w-screen-2xl max-w-0 scale-80 group-hover:scale-100 overflow-hidden transition-all duration-700 ease-in-out group-hover:px-2 group-focus:px-2">
                  {item.text}
                </span>
              </span>
            </div>
